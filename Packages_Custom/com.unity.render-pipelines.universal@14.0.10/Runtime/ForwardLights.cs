    ...
    public class ForwardLights
    {
        ...
        internal void PreSetup(ref RenderingData renderingData)
        {
            if (m_UseForwardPlus)
            {
                ...
                var itemsPerTile = visibleLights.Length + reflectionProbeCount; // 每个Tile最大光源数 = 附加光源数量 + 光照探针数量
                m_WordsPerTile = (itemsPerTile + 31) / 32; // 上取整

                // 将屏幕划分成不同的 Tile.
                //  划分原则: Tile尺寸尽可能小, 使用尽可能多的Tile数量(不超过Tile数量上限), 覆盖全屏幕
                ...
                // ZBin 划分
                //  划分原则:  使用尽可能多的ZBin, 覆盖整个Z值区域(远近裁剪面之间)
                ...
                // 使用 JobSystem 计算 Tile和ZBin内的光源索引
                ...
                // 分批次 Job计算  每段ZBin内的 最大最小光源索引并记录下光源索引
                //  每128个ZBin一个Job批次
